...
const enqsourceSlice = createSlice({
  ...
  reducers: {
    ...
    updateEnqSourceSuccess: (state, action) => {
      state.loading = false;
      state.enqsourceByIdData =
        action.payload === undefined
          ? current(state).enqsourceByIdData
          : current(state).enqsourceByIdData.map((Data) => (Data.id === action.payload.id ? action.payload : Data));
    },
    ...
    deleteEnqSourceSuccess: (state, action) => {
      ...
      state.enqsourceByIdData =
        action.payload === undefined
          ? current(state.enqsourceByIdData)
          : current(state.enqsourceByIdData).filter((option) => option.id !== action.payload);
...
export const {
  ...
  addEnqSourceFail,
  getEnqSourceById,
  getEnqSourceByIdSuccess,
  getEnqSourceByIdFail,
  totalCount,
  totalCountSuccess,
  totalCountFail,
  updateEnqSource,
  updateEnqSourceSuccess,
  updateEnqSourceFail,
  deleteEnqSource,
  deleteEnqSourceSuccess,
  deleteEnqSourceFail
