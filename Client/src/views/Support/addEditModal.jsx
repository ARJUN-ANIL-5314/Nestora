...
import { SprtTypeEnum } from 'utils/supportTypeEnum';
...
const AddEditModal = ({ formtype, data, handleClose }) => {
  ...
  const initialValues = {
    ...
    ticketNo: formtype === 'editform' ? data?.ticketNo || '' : '',
    imgUrls: formtype === 'editform' ? data?.imgUrls || '' : '',
    status: formtype === 'editform' ? data?.status || '' : ''
  };

  const validationSchema = Yup.object({
    sprtType: Yup.string().required('Type is required'),
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6}>
                ...
                >
                  ...
                  {Object.keys(SprtTypeEnum).map((key) => {
                    if (SprtTypeEnum[key].label !== 'Internal') {
                      return (
                        <MenuItem key={key} value={SprtTypeEnum[key].value}>
                          {SprtTypeEnum[key].label}
                        </MenuItem>
                      );
                    }
                    return null;
              ...
              <Grid item xs={12} sm={6}>
                ...
                <TextField
                  ...
                  // sx={{
                  //   ".css-1cb05yl-MuiInputBase-input-MuiOutlinedInput-input": {
                  //     height: '5px',
                  //     width: '100%',
                  //     border: 'none',
                  //     backgroundColor: 'white',
              ...
              <Grid item xs={12} sm={6} style={{ marginTop: '-60px' }}>
                ...
                {uploadError && <div style={{ color: 'red', fontSize: 12 }}>{uploadError}</div>}
              </Grid>
            </Grid>
