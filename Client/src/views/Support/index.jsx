...
import { Visibility, Delete, FilterList, CheckCircle, PersonOff, ExpandLess } from '@mui/icons-material';
...
import { getSupport, deleteSupport, updateStatus, totalCount } from 'container/SupportContainer/slice';
...
export default function Index() {
  ...
  const restrictedRolesForAdmin = ['vendor', 'vendorMngr', 'vendorAcnt', 'vendorOptr', 'licnseOptr', 'licnseMngr', 'licnseAcnt'];

  const shouldHideUpdateStatusButton = (row) => {
    if (userRole == 'admin' && restrictedRolesForAdmin.includes(row.supportFrom.role)) {
      return true;
    } else {
      return false;
    }
  ...
  const columns = [
    ...
    {
      name: 'TITLE',
      selector: (row) => capitalize(row.sprtTitle) || 'N/A'
    },
    ...
    },
    {
      ...
      cell: (row) => (
        <div style={{ width: '300px', display: 'flex', justifyContent: 'space-between' }}>
          ...
          {!shouldHideUpdateStatusButton(row) && isUserInSupportTo(userId, row.supportTo) && row.status !== 'completed' ? (
